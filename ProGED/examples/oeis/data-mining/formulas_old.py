...
s124 = ['A000124', ' Central polygonal numbers, Lazy Caterer (discovered)',
'',
' n(n+1)/2 + 1 ',
'a_n = a_{n-1} + n,',
...
s219 = ['A000219',     ' planar partitions ', 'no disc',
...
s292 = [ 'A000292',  'Tetrahedral',   ' (quick checked, discovered)',
'a(n) = n*(n+1)*(n+2)/6 ',
'a(n) = (n+3)*a(n-1)/n,',
...
s720 = ['A000720', ' PrimePi(n) aka. number of primes <= n',   '(no disc?)',
' a(n) ~ n/log(n) ,', 
...
s1045 = [ 'A001045',  'Jacobsthal sequence',   '(quick checked, discovered)',
'  a_n = a_{n-1} + 2*a_{n-2} ',
'   a_n = [2**n/3] ,',
'   a(n) ~ 2^n/3,',
'    a_n = 2^(n-1) - a(n-1),',
...
s1097 = [ 'A001097', 'aka. Twin primes',    '(quick checked, no disc.)',
' no formulas',]

s1481 = [ 'A001481',  '',   '(quick checked, no disc.)',
' a(n) ~ k*n*sqrt(log n) ']

s1615 = [ 'A001615',  '',   '(quick checked, no disc.)',
' no formulas', ]

s2572 = [ 'A002572',  'Number of partitions of 1 into n powers of 1/2',  ' (quick checked, no disc.)',
'',
'a(n) ~ c_1 * c_0^n,', 
'  a(n) ~ c_1 * c_0^n + c_3 * c_0^n * c_2^n,', 
]

s5230 = [ 'A005230', " a Stern's sequence",  ' (quick checked, no disc.)',
 "a(1) = 1, a(n+1) is the sum of the m preceding terms, where m*(m-1)/2 < n <= m*(m+1)/2 or equivalently m = ceiling((sqrt(8*n+1)-1)/2) ",
 " a(n) ~ 2*a(n-1)",
 ]

# s27642 = A027642  Denominator of Bernoulli number B_n   (quick checked, no disc.)
# [ no formula ]
# """
